...
use super::{FullNodeContent, HiddenNodeContent};
...
impl PartialEq for CompressedNode {
...
impl CompressedNode {
    ...
    #[allow(dead_code)]
    pub fn new_pad(blinding_factor: Secret, coord: &Coordinate, salt: Secret) -> CompressedNode {
...
impl From<HiddenNodeContent> for CompressedNode {
    fn from(node: HiddenNodeContent) -> Self {
        CompressedNode { commitment: node.commitment.compress(), hash: node.hash }
    }
}
...
impl Mergeable for CompressedNode {
    ...
    fn merge(left_sibling: &Self, right_sibling: &Self) -> Self {
        let parent_commitment =
            left_sibling.commitment.decompress().unwrap() + right_sibling.commitment.decompress().unwrap();

        // `hash = H(left.com | right.com | left.hash | right.hash`
        let parent_hash = {
            let mut hasher = Hasher::new();
            hasher.update(left_sibling.commitment.as_bytes());
            hasher.update(right_sibling.commitment.as_bytes());
        ...
        CompressedNode {
            commitment: parent_commitment.compress(),
            hash: parent_hash,
        }
    }
}
...
mod tests {
    ...
    fn new_leaf_works() {
        ...
        CompressedNode::new_leaf(liability, blinding_factor, entity_id, entity_salt);
    }
    ...
    fn new_pad_works() {
        ...
        CompressedNode::new_pad(blinding_factor, &coord, entity_salt);
    }
    ...
    fn merge_works() {
        ...
        let node_1 =
            CompressedNode::new_leaf(liability_1, blinding_factor_1, entity_id_1, entity_salt_1);
        ...
        let node_2 =
            CompressedNode::new_leaf(liability_2, blinding_factor_2, entity_id_2, entity_salt_2);

        CompressedNode::merge(&node_1, &node_2);
    }
}
