//! An implementation of the generic content type required for
//! [crate][binary_tree][`Node<C>`].
//!
//! This implementation contains the values in the [super][hidden_node]
//! implementation (Pedersen commitment & hash) plus the additional private
//! values (blinding factor and plain text liability). The private values are
//! included so that the total blinding factor & liability sum can be accessed
//! after tree construction. This node type should ideally not be used in
//! the serialization process since it will increase the final byte size and
//! expose the secret values.
//!
//! All the logic related to how to construct the content of a node is held in
//! this file.
...
impl FullNodeContent {
    ...
    /// Constructor.
    ///
    /// The secret `liability` realistically does not need more space than 64
    /// bits because it is generally used for monetary value or head count,
    /// also the Bulletproofs library requires the value to be u64.
...
impl Mergeable for FullNodeContent {
    ...
    ///
    /// The value and blinding factor of the parent are the sums of the two
    /// children respectively. The commitment of the parent is the
    /// homomorphic sum of the two children. The hash of the parent is
    /// computed by hashing the concatenated commitments and hashes of two
    /// children.
